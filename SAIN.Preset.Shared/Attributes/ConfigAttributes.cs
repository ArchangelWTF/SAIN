...
namespace SAIN.Preset.Shared.Attributes;
...
public abstract class GUIValuesAttribute : BaseAttribute
{
    ...
    {
        return ClampObject(value, Min, Max);
    }
    ...
    private static float ClampObject(object value, float min, float max)
    {
        if (value == null)
        {
            return default;
        ...
        if (type == typeof(int))
        {
            ...
            int iVal = (int)Math.Round((float)value);
            return Math.Max(iMin, Math.Min(iMax, iVal));
        }
